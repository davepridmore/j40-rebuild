...
OUT = ROOT / "docs" / "J40-integrated-cooling-pack-fabricator-specification-rev-h.docx"
SOURCE = ROOT / "docs" / "J40-integrated-cooling-pack-fabricator-specification-rev-h.md"
ASSET = (
    ROOT
    / "data"
    / "manual"
...
# Rev H definition with the same name is the active generator.
...
# Legacy Rev C dispatcher; the later Rev H dispatcher is the active definition.
...
# Some inherited deterministic drawing assets keep historical filenames,
# but the functions below replace the prior architecture with the Rev H
...
def finish_drawing_sheet(draw, code, description):
    ...
    draw.text((2050, 1570), "REV H", font=image_font(21, True), fill=hex_colour(BLUE))
...
def save_d01_complete_stack():
    image, draw = new_drawing_sheet(
        "D01",
        "COMPLETE REV H COOLING SYSTEM — TOYOTA-DONOR-LED ALL-FRONT STACK",
...
def save_d04_component_dimensions():
    ...
    drawing_note(draw, (1660, 1165, 2310, 1435), "REV H COMPONENT RULE", "50 body depth nominal with 57 mm beaded outlets. No side pack, side fan, service tower or extra width. Charge pipes use measured adapters/supports only; the cooler is removable and isolated. 50°C capability remains a complete-stack vehicle-test claim.", RED, "FDF2F1")
...
def save_d06_side_geometry():
    ...
    drawing_panel(draw, (45, 185, 2355, 900), "A — REV H ALL-FRONT PACK / M3–M5", fill="F8FAFC")
    x = 230
    y1, y2 = 350, 670
    items = [
...
def save_d07_fan_wiring():
    ...
    draw.text((90, 965), "Rev H uses two front motors, each with its own protected branch. No exposed fuse, holder or live fuse stud is permitted.", font=image_font(17), fill=hex_colour(MUTED))
...
def save_d10_procurement_fit_control():
    image, draw = new_drawing_sheet(
        "D10",
        "REV H VEHICLE-CENTRED FANS, PROCUREMENT AND PHYSICAL-FIT RELEASE",
        "C0 comes from paired chassis references; fixed nominal part bodies support sourcing, but real parts and the full-size dummy control cutting.",
    )
    ...
    rows = [
        ...
        ("FRONT FAN PARTS", "2 × 248; ≥258 rings; hubs at C0 ±133", GOLD, "F6EEDB"),
    ...
    drawing_panel(draw, (1160, 185, 2355, 780), "B — FANS CENTRED ON THE CAR / CHASSIS", fill="F8FAFC")
    centre_x = (1160 + 2355) / 2
    scale = 1.10
    condenser_width = 559 * scale
    condenser_height = 356 * scale
    cond_left = centre_x - condenser_width / 2
    cond_right = centre_x + condenser_width / 2
    cond_top = 275
    cond_bottom = cond_top + condenser_height
    draw.rectangle((cond_left, cond_top, cond_right, cond_bottom), fill="#DDECF3", outline=hex_colour(CYAN), width=6)
    draw.line((centre_x, 235, centre_x, 725), fill=hex_colour(PURPLE), width=4)
    radiator_width = 530 * scale
    dashed_rectangle(
        draw,
    ...
    ring_d = 258 * scale
    first_centre = centre_x - 133 * scale
    second_centre = centre_x + 133 * scale
    ...
    dimension_h(draw, cond_left, cond_right, 240, cond_top, "559 CONDENSER BODY", RED, 17)
    dimension_h(draw, first_centre, second_centre, 630, fan_y + ring_d / 2, "266 MOTOR C-C", RED, 17)
    centred_text(draw, (1205, 655, 2310, 690), "LEFT HUB C0 − 133  |  RIGHT HUB C0 + 133  |  PAIR MIDPOINT TO C0 ±2", image_font(15, True), PURPLE)
    centred_text(draw, (1205, 700, 2310, 730), "HUB HEIGHTS WITHIN ±2  |  530 RADIATOR ACTIVE-WIDTH BASIS DASHED  |  C0 COMES FROM CHASSIS", image_font(12, True), NAVY)
    ...
    drawing_note(
        ...
        (1525, 880, 2310, 1045),
        "W0 / R0 / M3",
        "First survey C0 from paired undamaged chassis references. W0 is the smallest clear post width. R0 is the COMPLETE radiator W×H×D, ears and lower locators—not only the active core. M3 fits real depth and service clearances.",
        PURPLE,
        "F3ECF8",
    )
    drawing_note(
    ...
    drawing_note(
        ...
        (1525, 1345, 2310, 1455),
        "FINAL RELEASE HOLD",
        "Do not buy final radiator/core or drill/coat until C0/B0/H0-L/H0-R/W0/P0/R0, BRKT, M1–M6, F1 and E2/E2a are signed. AI images are not evidence.",
        RED,
        "FDF2F1",
    )
    finish_drawing_sheet(draw, "D10", "VEHICLE C0 / FAN HUBS ±133 / DEPTH BUDGET / FIT RELEASE")
    image.save(ASSET / "rev_h_d10_vehicle_centreline_control.png", quality=96)
...
DRAWING_SHEETS = [
    ("D01", "Retained Rev G complete all-front stack", "rev_c_d01_complete_stack.png", "Three-core all-front airflow path and installed-performance targets."),
    ...
    ("D10", "Vehicle-centreline and physical-fit control", "rev_h_d10_vehicle_centreline_control.png", "Chassis-derived C0, fan hubs at C0 ±133 mm, nominal purchase dimensions, depth budget and signed fit-release boundary."),
]


def add_page_number(paragraph):
    paragraph.alignment = WD_ALIGN_PARAGRAPH.RIGHT
    run = paragraph.add_run("J40 Cooling System  |  Rev H  |  ")
    set_run(run, 9, False, MUTED)
    field = OxmlElement("w:fldSimple")
    field.set(qn("w:instr"), "PAGE")
    paragraph._p.append(field)
...
def add_landscape_drawing_appendix(document):
    add_heading(document, "Appendix A. Rev H control and retained layout-reference drawings", 1)
    add_paragraph(
        document,
        "D10 is the new Rev H vehicle-centreline and release-control sheet. D01–D09 are retained Rev G layout-reference "
        "sheets. Red values are fixed performance/release limits. Purple dimensions and interfaces are measured from the "
        "vehicle or actual component. The Rev H written requirements and D10 control; do not scale the images.",
    )
    section = document.add_section(WD_SECTION.NEW_PAGE)
    section.orientation = WD_ORIENT.LANDSCAPE
    section.page_width = Inches(11)
...
# The active Rev H builder below supersedes the retained prior-revision
# reference above; only this function is invoked by the script entry point.
def build_document():
    # Preserve the released Rev G D01–D09 assets. Rev H changes only the
    # chassis-centreline control sheet; regenerating the shared older assets
    # would silently relabel historical references.
    save_d10_procurement_fit_control()
    document = Document()
    configure_document(document)
    ...
    set_run(r, 27, True, NAVY)
    p = document.add_paragraph()
    r = p.add_run("Toyota 2H • R134a A/C • Rev H Toyota-donor-led all-front stack • best-effort / test-gated")
    set_run(r, 12.5, False, MUTED)
    p = document.add_paragraph()
    r = p.add_run("REV H  |  01 AUGUST 2026  |  ALL DIMENSIONS IN mm")
    set_run(r, 10, True, BLUE)
    add_callout(
        document,
        "DESIGN BASIS — NOT A GUARANTEED 50°C OR NO-DERATE RATING",
        "Rev H is a simple best-effort all-front architecture using standard service parts where they are locally obtainable or sourceable. Vehicle centreline C0—not a radiator, photograph or accessory—controls the two front fan positions. It may be described as 50°C-capable or no-derate only after measured fit, component evidence and the complete-vehicle acceptance test pass.",
    )
    p = document.add_paragraph()
    ...
    sourcing_table = add_table(document, ["SOURCE CLASS", "QTY / ITEM", "EXACT ITEM / SEARCH REFERENCE", "PAKISTAN ACTION"], [
        ...
        ("FABRICATE LOCALLY", "Measured interfaces only", "C0 datum fixture; 2 side rails; 2 direct 4 mm top ears; 2 lower saddles; vehicle-centred twin-pusher shroud; rear shroud if needed; tabs, seals/screen; 2 separate electrical brackets/hoods and adapters", "Only after real parts, chassis-derived C0, rigid top-hole template and full-size dummy pass. No fan offset, crossbar, shared tray, new chassis hole, slot or forced alignment."),
    ...
    add_heading_new_page(document, "1. Rev H packaging — Toyota donor parts, bracket-first measured fit", 1)
    add_callout(document, "CONTROLLED ARCHITECTURE", "Survey vehicle longitudinal centreline C0 from paired chassis references. Grille → matched Prado 120/GX470-family Denso pusher hubs at C0 − 133 mm and C0 + 133 mm in a custom shroud → 500 × 180 × 50 charge cooler → ≥10 mm gap → 559 × 356 × 21 condenser → 15 mm target gap (10 mm absolute minimum) → HJ47/2H-pattern recored/custom radiator using a 530 × 435 × 64 active-core thermal basis only, with complete envelope controlled by R0 → sealed full-face mechanical puller shroud. Rev H has exactly three fans: two front electric pushers and one retained rear engine-driven puller. Prado/GX sourcing is for matched fan motors, blades, connectors and pigtails only; no Prado radiator or direct-fit donor-shroud assumption.", "F3ECF8", PURPLE)
    add_picture(document, ASSET / "rev_g_ph01_complete_wide_exploded_v2.png", 6.20)
    add_paragraph(document, "PH01 — Retained Rev G complete wide split visual. Read the main row from left to right as the installed front-to-rear order: twin Toyota/Denso electric-pusher module, short central charge cooler, condenser, engine radiator, then the sealed rear shroud and large retained mechanical puller. Uprights, rails, direct ears, lower saddles, drier, protected electrical enclosures, seals, connectors and fasteners are also separated for identification. Exploded positions do not authorise installed side placement or extra width. D10/C0 controls final front-fan centring; PH05/D09 and E2/E2a control the electrical installation. This is an AI layout aid only; signed measurements, actual components and the accepted full-size dummy govern manufacture.", italic=True)
    add_heading_new_page(document, "1.1 Rev H component assembly", 2)
    add_picture(document, ASSET / "rev_g_ph02_toyota_donor_assembled.png", 5.40)
    add_paragraph(document, "PH02 — Retained Rev G off-vehicle central cooling-module assembly. It shows two equal Toyota/Denso 248 mm electric pushers and the retained engine-driven puller behind the radiator: exactly three fans total. Because the chassis is absent, PH02 does not establish vehicle centring. D10/C0 and F1 control the final hub coordinates. The side rails, direct top ears and lower saddles belong to the removable radiator module. PH03C/PH04 illustrate the original-hole attachment; PH05/D09 and E2/E2a control the separate upper/rear electrical installation.", italic=True)
    add_heading_new_page(document, "1.2 Normal opaque front-installed appearance", 2)
    add_picture(document, ASSET / "rev_h_ph03a_interface_opaque_centered.png", 6.20)
    add_paragraph(document, "PH03A — Fully opaque Rev H front-installed appearance on the later J40 structure. There is no transparent or cutaway area. The two equal electric pusher fans and compact front stack are visible between the existing uprights; the retained engine-driven mechanical puller remains installed behind the opaque radiator and is therefore correctly hidden from this direction. PH03A is contextual only. It does not prove the ±2 mm centring tolerance or release mounting geometry; D10/F1, the chassis-derived C0, signed measurements and full-size dummy control.", italic=True)
    add_heading_new_page(document, "1.3 Fully opaque engine-side rear view", 2)
    add_picture(document, ASSET / "rev_h_ph03b_opaque_engine_side_rear.png", 6.20)
    add_paragraph(document, "PH03B — Fully opaque view looking forward from the engine bay. It plainly shows the one large retained Toyota 2H engine-driven mechanical puller centred in a sealed full-face rear shroud, with the copper/brass radiator immediately ahead. The two 248 mm electric pushers, charge cooler and condenser are on the grille side and are correctly hidden by the opaque radiator. The Relay and MIDI enclosures are closed; no fuse, holder or live stud is exposed. This is an illustrative appearance view only; do not derive exact fan diameter, hose routing, connector placement or mounting dimensions from it.", italic=True)
    add_heading_new_page(document, "1.4 Controlled interface cutaway — centring and hidden layers", 2)
    add_picture(document, ASSET / "rev_h_ph03_interface_cutaway_centered_final.png", 6.20)
    add_paragraph(document, "PH03C — Corrected Rev H installed interface cutaway on the later J40 front structure. The midpoint between the two equal, level front-pusher hubs visibly coincides with the centred rear mechanical-fan hub and the car/chassis longitudinal centreline. D10/F1 controls fabrication at C0 − 133 mm and C0 + 133 mm, 266 mm centre-to-centre, with pair midpoint and hub heights each within ±2 mm. The cutaway visibly accounts for the short black charge cooler with two large 57 mm charge-air ports, silver condenser with two smaller A/C fittings, copper/brass engine radiator with upper and lower coolant necks plus filler/cap, overflow and drain, sealed rear shroud and large engine-driven mechanical puller as separate layers. At each side, the M8/washer stack passes through the original top-return hole and sleeved EPDM isolator into one independent 4 mm ear connected directly to that radiator side rail; the lower saddle beneath the same rail carries weight. No transverse carrier or intermediate bracket. This AI image is illustrative only; C0, the rigid two-hole template, B0/H0-L/H0-R/W0/P0/R0 measurements and full-size dummy govern fit.", italic=True)
    add_heading_new_page(document, "1.5 Existing-hole attachment close-up", 2)
    ...
    add_heading_new_page(document, "1.6 Closed fuse/relay boxes on independent brackets", 2)
    ...
    add_table(document, ["ITEM", "REV H CONTROL"], [
        ...
        ("Total fans", "Exactly 3: two vehicle-centred front electric pushers plus one retained rear engine-driven mechanical puller. No third electric or fourth fan in Rev H."),
        ("Front pusher geometry", "Survey chassis C0; hubs at C0 − 133 / C0 + 133 mm; 266 mm C-C; pair midpoint and hub-height mismatch each ≤2 mm."),
        ("Front pusher airflow", "Two matched Toyota/Denso 248 mm candidates in one sealed module; prove ≥3,000 m³/h installed through the complete three-core stack at 13.5 V."),
        ("Six main circuit connections", "Radiator: 2 large coolant necks. Condenser: 2 smaller refrigerant fittings. Charge cooler: 2 large 57 mm charge-air ports. Do not interchange circuits."),
    ...
    add_table(document, ["PART", "REQUIRED"], [
        ...
        ("Twin pusher module", "Two matched Prado 120/GX470-family Denso 248 mm motor/blade/connector sets; ≥258 mm rings. Establish C0 from paired chassis references; hubs C0 − 133 / C0 + 133 mm; 266 mm C-C; midpoint and height mismatch each ≤2 mm. Custom sealed symmetric shroud; independent fuse/relay branches."),
        ("Charge-air cooler", "New universal bar-and-plate body exactly 500 W × 180 H × 50 D with exactly two 57 mm beaded charge-air ports; ≥15 kW, IAT ≤80°C and route Δp ≤10 kPa are installed-test targets."),
        ("Condenser", "New 14 × 22 nominal R134a parallel-flow body 559 W × 356 H × 21 D with exactly two smaller refrigerant fittings (#8 inlet / #6 outlet preferred) on isolated removable tabs; ≥10 mm ahead and 15 mm target / 10 mm absolute minimum behind."),
        ("Radiator / puller", "HJ47/2H-pattern recore/custom radiator with exactly two large engine-coolant hose necks (upper and lower), plus filler/cap, overflow nipple and lowest drain. 530 W × 435 H × 64 D is the active-core thermal basis, not the finished radiator size. Record R0 as complete W × H × D including tanks, seams, necks, filler, overflow, drain, ears and lower locators. Build only if complete R0 fits signed W0/H0-L/H0-R/P0 and bonnet/bridge. Retain the Toyota mechanical puller and sealed full-face shroud. Do not buy a Prado radiator."),
    ...
    add_callout(document, "BUY SAMPLES FIRST", "Do not buy the final radiator or cut radiator ears from catalogue dimensions. First survey C0 from paired chassis references, make one rigid template of both original top holes and record C0/B0/H0-L/H0-R/W0/P0. Source physical fan, radiator-pattern and electrical-box samples; record complete R0 and service envelopes; then prove the full-size dummy, the C0-centred shroud and both top bolts entering fully by hand with zero pull.", "FFF8E8", GOLD)
    ...
    add_paragraph(document, "Full seller-facing list: docs/j40-rev-h-toyota-donor-cooling-pack-purchase-list-20260801.md. Machine-readable schedule: data/manual/fabrication/front_cooling_stack_rev_c/rev_h_toyota_donor_purchase_list.csv.", italic=True)
    add_picture(document, ASSET / "rev_h_d10_vehicle_centreline_control.png", 6.15)
    add_paragraph(document, "D10 — Vehicle-centreline, procurement and fit control. Survey C0 from the chassis and locate the front fan hubs at C0 − 133 mm / C0 + 133 mm; actual complete parts, signed vehicle dimensions and the accepted dummy still control fabrication.", italic=True)

    add_heading_new_page(document, "4. Airflow, charge-air and A/C requirements", 1)
    add_picture(document, ASSET / "rev_c_d04_component_dimensions.png", 5.15)
    add_bullets(document, [
        "Rev H has exactly three fans: two electric pushers symmetric about chassis-derived C0 at −133/+133 mm and the retained engine-driven mechanical puller behind the radiator. No additional fan is released.",
        "The charge cooler has exactly two large 57 mm charge-air ports, the condenser exactly two smaller refrigerant fittings, and the radiator exactly two large engine-coolant necks plus filler/cap, overflow and drain. These are three different circuits.",
    ...
    add_bullets(document, [
        ...
        "Transfer chassis-derived C0 to the rigid aperture fixture and fan-shroud template before cutting the shroud. C0 does not move if a core, drier, A/C fitting or electrical box is asymmetric.",
    ...
    add_bullets(document, [
        "Rev H uses exactly two front electric motors plus the retained rear mechanical puller. Do not substitute one pusher or add a third electric/fourth fan without a new controlled revision.",
    ...
    gates = [
        ("C0", "vehicle longitudinal centreline", "establish from paired undamaged chassis/frame references; record left/right survey values; mark on aperture, rigid fixture and shroud template; do not derive from a core, photograph or accessory"),
        ...
        ("F1", "twin pusher envelope and vehicle centring", "equal 248 mm candidates; ≥258 mm rings; hubs C0 − 133 / C0 + 133 mm; exactly 266 mm C-C; pair midpoint within ±2 mm of C0; hub heights within ±2 mm; no sideways offset or contact"),
    ...
    add_numbers(document, [
        "Source only Phase A physical samples. Establish chassis-derived C0; measure B0/H0-L/H0-R/W0/P0/R0 and M1–M6; prove the BRKT, F1–F2 and separate E2/E2a dummies before buying Phase B final cores.",
        "Mark C0 on the aperture, rigid fixture and fan-shroud template. Set the two front fan hubs at C0 − 133 mm and C0 + 133 mm; prove pair midpoint and hub-height tolerances within ±2 mm. Make one rigid 1:1 template that captures both original top holes. Make a full-size radiator dummy including complete tanks, seams, both coolant necks, filler, overflow, drain, ears, lower locators, cap/tool sweep and hose routes. Also mock each real electrical box separately with its own cover/lid sweep, cable bends, hood and removal path.",
    ...
    records = [
        ("[ ]", "C0 / B0 / H0-L / H0-R / W0 / P0 / R0 / M1–M6 / BRKT / F1–F4 / E1–E2a / T1–T3 as-measured sheet", ""),
        ("[ ]", "C0 survey evidence and fan-hub coordinates: left −133 mm / right +133 mm; midpoint and height tolerances", ""),
    ...
    add_heading(document, "Controlled references", 2)
    add_bullets(document, [
        "docs/J40-integrated-cooling-pack-fabricator-specification-rev-h.md (controlled Rev H source specification)",
        "docs/j40-rev-h-toyota-donor-cooling-pack-purchase-list-20260801.md (seller-facing purchase and receiving list)",
        "data/manual/fabrication/front_cooling_stack_rev_c/rev_h_cooling_pack_visual_prompts.md (AI visual prompt/output record; illustrative only)",
        "SAE J1994 heat-exchanger heat-transfer/pressure-drop testing.",
        "SAE J1339 engine-cooling fan performance.",
        "SAE J1393 heavy-duty vehicle cooling test procedures.",
    ])
    add_landscape_drawing_appendix(document)
    document.core_properties.title = "J40 Integrated Radiator and Cooling System — Pakistan Fabricator Specification — Rev H"
