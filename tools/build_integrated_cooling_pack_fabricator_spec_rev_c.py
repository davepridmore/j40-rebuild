...
def build_document():
    ...
    add_paragraph(document, "PH03 — Proposed installation on the later chassis photo. At each side, the same M8 through-bolt passes through the original frame top-return hole, a sleeved EPDM isolator and the short direct radiator ear; that ear is fixed directly to its removable radiator side rail. A lower saddle beneath the same rail carries the weight. The two enlarged insets show this continuous frame-to-rail bolt path. There is no transverse radiator carrier or intermediate bracket. This AI image is illustrative only; the rigid two-hole template, B0/H0-L/H0-R/W0/P0/R0 measurements and full-size dummy govern fit.", italic=True)
