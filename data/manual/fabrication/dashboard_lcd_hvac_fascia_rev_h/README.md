...
The straight-on bare-shell overlay is the controlling visual placement view for the top-right outlet (V4). With the nominal Ø22.5 head, the drawn V4-rim-to-nearest-selector gap is **25.75 mm**, so it passes the **≥20 mm** face-layout target. The smaller **9 mm** relationship is to the conservative rectangular bank/label envelope, not to the selector head. M6 must repeat the head, label and lever-sweep check using the bought controls. M8/M9 must prove at least **10 mm** from V4's actual retainer, neck, elbow and duct to selector contact blocks, terminals and wiring. V4 must not be moved farther right: its nominal end lands are already 6.5 mm visible-face / 12.5 mm Ø75 opening-reference. See `rear_envelope_fit_audit_20260801.md`; the full-depth M1-M9 mock-up controls release.
