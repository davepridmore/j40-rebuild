...
> **SUPERSEDED FOR HVAC FABRICATION — updated 2026-08-02:** the active [Rev I V15 specification](../dashboard_lcd_hvac_fascia_rev_i/README.md) uses a compact two-takeoff evaporator and exactly two high outer/end occupant outlets, one at each fixed end. Its visual target retains the geometrically centred 9-inch LCD, large asymmetric plate-and-knob OEM glovebox and speedometer, deletes and closes over the separate smaller ashtray, preserves the seven-selector-plus-hazard bank, purchased MOMO wheel, original-height silhouette and factory steering-column relief, and starts with the cluster in its installed-photo relationship immediately above the column shroud. The signed vehicle trace, not the rendering, controls exact geometry. Do not cut the Rev H V1-V4 apertures or order its four-branch duct package. Retain this package only as the LCD/control reference and historical four-outlet record.
...
- At the extreme right, fully outside that keep-out, exactly seven industrial rotary selectors plus one separate red hazard form **two rows of four**. Nominal columns are **X=(1120.0, 1160.0, 1200.0, 1240.0)**, with top **Y=78** and bottom **Y=20**: 40 mm horizontal / 58 mm vertical pitch. The V6 bank is compacted, shifted right and lowered within the same shallow Y=3 control channel; dashboard height is unchanged. Schneider Harmony XB4 reference: **Ø22.5 panel cut** and **68 mm rear envelope**. Head/lever sweep, anti-rotation, right-outlet duct route, rear stacks and driver clearance remain M6/M8/M9 HOLD. Engrave labels 3 mm high with black infill.

## V4 / right-side rear-envelope control

The V6 straight-on installed view supersedes the earlier views for control-bank placement; the straight-on bare-shell overlay remains the visual placement control for the fixed top-right outlet (V4). With the nominal Ø22.5 head, the drawn V4-rim-to-nearest-selector gap is **35.75 mm**, so it passes the **≥20 mm** face-layout target. The smaller **19 mm** relationship is to the conservative rectangular bank/label envelope, not to the selector head. M6 must repeat the head, label and lever-sweep check using the bought controls. M8/M9 must prove at least **10 mm** from V4's actual retainer, neck, elbow and duct to selector contact blocks, terminals and wiring. V4 must not be moved farther right: its nominal end lands are already 6.5 mm visible-face / 12.5 mm Ø75 opening-reference. See `rear_envelope_fit_audit_20260801.md`; the full-depth M1-M9 mock-up controls release.
