...
| --- | --- | --- | --- | --- |
| `dashboard_lcd_hvac_fascia_rev_i` | Dashboard / infotainment / A/C HVAC | Purchased Pioneer DMH-AP6650BT incorporated into the owner-selected V35 centre-cassette target; CNC quotation plus full-size/full-depth disposable template only; production geometry and cutting held for M1-M9, with M10 required before commissioned vehicle handoff | `dashboard_lcd_hvac_fascia_rev_i/README.md` | Centre-only removable cassette replacing the ashtray area while the OEM outer fascia, formed contours, large asymmetric plate-and-knob glovebox, complete Toyota speedometer and factory RHD column scallop remain unchanged. The cassette carries the purchased 9-inch Pioneer receiver and a single line of seven bought selectors plus separate red Hazard; exactly two high outer/end chrome/silver outlets are separate retained-panel operations. Published 188 x 108 x 37 mm main-chassis dimensions inform the depth study, but the exact screen face, mount, connector and complete service envelope remain held for M4/M5 measurement and M9 buck proof. |
...
| `suspension_wood_cribbing_rev_a` | Suspension setup | Fabrication complete and closed; retained reference package | `suspension_wood_cribbing_rev_a/j40_suspension_wood_cribbing_rev_a_dimension_sheet.pdf` | Received hardwood cribbing blocks and exact wedge chocks for suspension/brake support setup; includes interactive 3D visualisation. |
