...
| --- | --- | --- | --- | --- |
| `dashboard_lcd_hvac_fascia_rev_i` | Dashboard / infotainment / A/C HVAC | Owner-selected V15 visual target; CNC quotation plus full-size/full-depth disposable template only; production metal and vehicle cutting held for M1-M10 | `dashboard_lcd_hvac_fascia_rev_i/README.md` | Complete visible-face replacement targeting the original shallow dashboard height. Retains the original large asymmetric plate-and-knob glovebox and complete Toyota speedometer, with the cluster lower lip immediately above the steering-column shroud on their directly traced vehicle axis. Deletes and closes over the separate smaller ashtray, centres a true 9-inch active image, uses exactly two high outer/end silver circular outlets, and provides seven labelled selectors plus a separate red Hazard. Every final perimeter, aperture and behind-dash feature remains controlled by the vehicle trace and bought-component measurements. |
...
| `front_cooling_stack_rev_a` | Fabrication / radiator / A/C HVAC | Mock-up release with component compatibility gates and Pakistan-local buy sheet | `front_cooling_stack_rev_a/README.md` | Two full-height chassis/front-support uprights carrying the HJ47/2H radiator, parallel-flow A/C condenser, optional pusher fan, and receiver-drier as separate rubber-isolated components; includes compressor compatibility gate for the existing Sanden-type compressor and a local parts selection CSV. |
