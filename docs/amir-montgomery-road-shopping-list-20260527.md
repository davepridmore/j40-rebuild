...
| Glow plugs | **VERIFY, THEN BUY.** Engine is Toyota `2H-117299`. Remove and photograph one fitted plug and the system nameplate; verify voltage, thread, seat, reach and tip projection before requesting six matched 2H plugs. `19850-68030` / `19850-68060` remain candidate references only until that check. | Block stamp, system voltage/nameplate, old-plug markings and dimensions, proposed box/part number, count x6, price and return terms. | Buy only a verified 2H/as-fitted-system match; serial identity alone does not prove the installed glow system or superseded part number. |
