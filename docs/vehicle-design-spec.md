...
Design interpretation:
- the active Rev I V35 direction replaces only the ashtray/centre area with a restrained body-colour removable centre cassette. The OEM glovebox, J40 speedometer/cluster, RHD column scallop, full-width lower edge and all existing formed contours remain unchanged; default cassette extension/drop is zero, and any future extension is centre-only after physical measurement and owner approval
- the cassette carries the provisional 9-inch module (211.10 × 126.50 mm; active image provisionally 198.91 × 111.89 mm), exactly two outer Ø87 mm target vents centred in retained measured clear lands, and no centre/lower vents. All aperture, rear-mount, duct and position data remain HOLD pending physical traces, actual parts and a complete rear buck; the active image, V35 quotation specification, coordinates and exact-scale elevation are controlled by `data/manual/fabrication/dashboard_lcd_hvac_fascia_rev_i/README.md`
