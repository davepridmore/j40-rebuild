...
| `RP-COOL-006` | Loose May 2 cooling sample | Formed pipe rubber connector/coupler hoses | May 2 template photos | Two EPDM coolant connectors visible on the sample; measure each pipe-end OD, mating spigot OD, cut length, overlap, clamp OD, and kink clearance in `mm` before order. |
...
### Pakistan Local Market Wording

Use these shop-facing words when asking for quotes. The numbers below are stock buy lengths; the workshop still cuts or bends to the measured sample/route.

| Circuit | Ask the shop for |
| --- | --- |
| Upper/lower radiator hoses | "HJ47 / 2H Land Cruiser upper and lower radiator rubber hoses, EPDM, not universal water pipe. Match Toyota `16571-68020` upper and `16572-68020` lower only as shape references." |
| Formed metal coolant pipe | "Coolant/radiator metal pipe made from sample: minimum `750 mm` tube blank, or quote `1000 mm` stock if you sell by meter/foot. Expected `28-30 mm OD`, `1.2-1.6 mm` wall, beaded ends, pressure-test before paint." |
| Formed pipe rubber couplers | "Two EPDM radiator rubber pipe pieces, `500 mm` each, working `28-30 mm ID`, to cut from sample. If straight pipe kinks, match with molded EPDM bend pieces." |
| Heater hose | "EPDM heater pipe, `16 mm / 5/8 inch` ID working size, one `400 mm` inlet and one `280 mm` outlet, or give `1000 mm` stock to cut." |
| Diesel rubber hose | "Diesel pipe only: `8 mm ID x 3000 mm` feed, `6 mm ID x 2000 mm` return, `3.2-3.5 mm ID x 1000 mm` injector leak-off. No washer pipe, no coolant pipe." |
| Fuel hard line | "Automotive bundy pipe or CuNi/Cunifer, `8 mm OD x 5000 mm` feed and `6 mm OD x 5000 mm` return. No bare copper." |
| Brake hard line | "Brake pipe coil `4.75 mm / 3/16 inch OD x 25 ft / 7600 mm`, with correct brake flare nuts/unions after matching the old sample. No bare copper." |
| Clutch hard line | "Brake/clutch hydraulic pipe blank `4.75 mm / 3/16 inch OD x 1500 mm` if the old clutch hard line is bad, cut and flared to sample." |
| Vacuum/breather | "Reinforced vacuum pipe `10-12 mm ID x 2000 mm`; oil-resistant breather pipe `16-19 mm ID x 1000 mm`. Do not use soft water pipe for brake booster vacuum." |
| Clamps/supports | "Smooth-band or constant-tension clamps for coolant, rolled-edge fuel-injection clamps for diesel, rubber-lined P-clips for hard lines every `300-400 mm`." |
...
| `RP-COOL-005` | Fabricate from sample | Send the old formed coolant pipe plus the six May 2 photos. Start with `750 mm` minimum tube blank, or quote `1000 mm` stock if the fabricator sells by meter/foot, then trim to the physical sample's measured centerline length. Fabricator must copy tube OD, wall, bend radii, bend clocking, offsets, beaded ends, clamp lands, and hose overlap in `mm`. | Final centerline length, tube OD, hose ID, wall thickness, bend centerlines/radii, bead height, clamp land length, clearance to fan/belts/radiator/body. | Bench pressure-test before coating; vehicle dry-fit passes; coat only after fit approval. |
| `RP-COOL-006` | Replace connector hoses | Ask for `2 x 500 mm` EPDM radiator rubber pipe blanks, working `28-30 mm ID`, then cut only after measuring pipe-end OD/bead, mating spigot OD, connector free/cut length, hose ID/OD, overlap, clamp band width, and bend/kink clearance in `mm`. Use molded EPDM connectors if straight hose would kink. | Connector A and B IDs/ODs, pipe-end ODs/beads, mating spigot ODs, finished lengths, overlap, clamp OD, dry-fit bend clearance. | No seepage, swelling, clamp-edge cutting, or kink after dry-fit and cooling pressure test. |
...
- formed coolant pipe connector hose ends, mating spigots, and installed kink clearance
