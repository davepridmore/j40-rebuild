...
### Rev E Standard-Parts / Compact-Pack Release — supersedes nominal stack assumptions

The current controlled fabrication handoff is [J40 Integrated Cooling Pack Fabricator Specification — Rev E](J40-integrated-cooling-pack-fabricator-specification-rev-c.md). It supersedes earlier nominal front-stack assumptions, including a fixed `530 × 435 × 64 mm` core, a dual-9-inch/free-air-fan target, any side service tower and any arrangement that puts the turbo intercooler in front of the condenser/radiator.

Build the engine radiator and A/C condenser as a measured-fit **main path** only: grille → A/C pusher frame → condenser → `≥15 mm` gap → radiator → sealed full-face shroud → engine-driven puller. Start with Toyota HJ47/2H radiator and hose patterns, then use two matching Toyota/Denso Prado 120/GX470-family `248 mm` motors/blades in one shallow custom shroud if their physical samples pass. Put both motor centres at the same Y and at `W-active/2 ±133 mm` (`266 mm` C-C), so the pair is horizontally central with equal uncovered fin bands. The drier belongs behind an upright and the front-fan relays/fuses above or behind the carrier—never in a side strip. Final core selection must prove `≥115 kW` continuous heat rejection and `≥130 kW` for ten minutes at `50°C` dry-bulb grille inlet with A/C on; also prove a `52°C` ten-minute heat soak and hot restart. The engine-driven puller must deliver `≥9,000 m³/h` installed at `125 Pa` and `1,500 rpm`; the independently fused/relayed A/C pushers must deliver `≥3,000 m³/h` installed at `75 Pa` and `13.5 V`.

The future turbo charge cooler is a separate side/wing system, not a third layer in the main pack: sealed fresh inlet → side charge cooler → dedicated `≥2,500 m³/h` installed fan at `75 Pa` → separate hot exit. Package its long dimension fore-aft and smallest complete dimension laterally, targeting no more than `160 mm` projection beyond the selected upright; the complete M8 template and installed-flow test control, not the target. Its release targets are `≥15 kW` rejection at the Rev E charge-air duty, manifold IAT `≤80°C`, and complete charge-route pressure loss `≤10 kPa` (target `≤7 kPa`). After all Rev E thermal/airflow gates pass, cooling must not force boost derate inside the `150 bhp` crank thermal-design envelope. This is not approval of 150 bhp, arbitrary boost, fuelling, turbo-map speed, EGT, drive-pressure or driveline capacity; turbo calibration still starts at `5-7 psi`, with `8-10 psi` only after logged evidence.
