...
| --- | --- |
| Occupant outlets | Two matched, generously sized directional outlets centred in retained measured outer clear lands, one independently aimable toward the passenger and one toward the driver; V35 target family is Ø87 visible face / Ø75 rear mount / ≤ approximately 22 mm louver-body projection; no central/lower outlets or pods |
| LCD / fascia relationship | Fit the bought LCD's measured active image—provisionally 198.91 × 111.89 mm within a 211.10 × 126.50 × 5.0 mm module—into the physical free centre field of the V35 removable cassette; determine the two vent positions separately from measured retained outer lands and rear packages |
...
| Fabrication status | V35 production geometry HOLD until physical fascia traces, louver/LCD/control components, evaporator case, fittings, drain, wiring, service-removal path and a complete rear buck are measured/proven |

Treat each vent as separate front-face, panel-mount and duct interfaces. Ø87 controls only the visible face. Ø75 is a target rear mounting boss/barrel and is **not** a released fascia hole or hose size. The approximately 22 mm figure runs from the finished rear fascia plane to the rear of the louver body only; measure the retainer, spigot, clamp and first bend separately. Release the aperture only from all-four-part M7 measurements and a same-1.5-mm-CR4, final-finish fit coupon. If the measured maximum boss is ≤75.00 mm and the part is slip-through/back-retained, the first conditional coupon may be Ø75.50 mm +0.25/−0.00; other retention types require their own proven size.

The [Rev I V35 image](../data/manual/fabrication/dashboard_lcd_hvac_fascia_rev_i/dashboard_rev_i_v35_registered_center_cassette_overlay.png), [quotation specification](../data/manual/fabrication/dashboard_lcd_hvac_fascia_rev_i/dashboard_rev_i_v35_quotation_cnc_spec.md), [coordinates](../data/manual/fabrication/dashboard_lcd_hvac_fascia_rev_i/dashboard_rev_i_v35_provisional_coordinates.csv) and [exact-scale elevation](../data/manual/fabrication/dashboard_lcd_hvac_fascia_rev_i/dashboard_rev_i_v35_provisional_front_elevation.svg) are the current quotation baseline. Do not use them as production coordinates. The OEM glovebox, cluster, RHD column scallop, full-width lower edge and formed contours remain unchanged; only the ashtray/centre area becomes a zero-drop cassette. After the physical traces and M7 measurements, prove both complete outlet packages and the cassette rear stack in one rigid rear buck.

Before purchasing the replacement, obtain tape-measure photos of overall case width, height and depth; the two outlet neck ODs and rear projection; refrigerant fitting side and thread/type; drain position; mounting tabs; return-air face; blower current; thermostat/freeze control; TXV; and stated airflow/capacity. Cardboard-mock that complete envelope in the J40 with the glovebox, cluster, column, knees and lever sweeps represented. Measure the dashboard outlet spigot OD/ID, bead and engagement separately and provide a smooth sealed adapter to the evaporator duct size where required.
...
Exactly seven bought long-handle selectors plus a separate red hazard are consolidated in one labelled two-row, four-column bank at the extreme right, at `48 mm` nominal horizontal and `58 mm` vertical pitch. The top row is `WIPERS | LIGHTS | SPOTS | AUX`; the bottom row is `BLOWER | A/C | FUEL STOP | HAZARD`. `WIPERS`, `LIGHTS`, `BLOWER` are 3-position; `SPOTS`, `AUX`, `A/C`, `FUEL STOP` are 2-position. `FUEL STOP` is a low-current RUN/STOP request only after EEI-003 proves the diesel fuel-stop logic; key OFF remains authoritative and the manual stop cable remains the emergency fallback. The bank does not duplicate OEM indicator/dip/horn controls, keyed ignition, washer input or winch lever. Cabin temperature/blend remains with the measured evaporator thermostat/controller and is not an eighth selector. The blower selector provides `OFF / LOW / HIGH`; the donor medium-speed tap remains unused unless a separate bench-proven controller redesign is approved. Gates M6, M8 and M9 must prove that selector heads, lever sweeps and rear contact stacks clear both right-hand duct branches, the retained cluster, steering column and driver controls. Only the disposable full-size template is released before gates M1-M10; production vehicle metal, LCD, selector, hazard and vent apertures remain HOLD until physical tracing, bought-part measurement and rear-envelope mock-up are signed.
