...
| --- | --- |
| Occupant outlets | Two matched, generously sized directional outlets on one high datum in the fixed outer fascia regions, one independently aimable toward the passenger and one toward the driver; owner-supplied V32 target family is Ø87 visible face / Ø75 rear mount / ≤ approximately 22 mm louver-body projection; V32 moves both faces slightly higher/inward for visual balance, subject to full-depth clearance; no central/lower outlets or pods |
| LCD / fascia relationship | Centre the bought LCD's measured active image—provisionally 198.91 × 111.89 mm within a 211.10 × 126.50 × 5.0 mm module—on the signed complete fascia face in both axes; determine the two vent positions separately from the measured fixed end lands and rear packages, not from their midpoint |
...
The [Rev I V32 image](../data/manual/fabrication/dashboard_lcd_hvac_fascia_rev_i/dashboard_rev_i_v32_inboard_high_87mm_outlets.png) is the current front-view appearance study. Do not scale its centre coordinates. After the M1/M2 trace and M7 vent measurements, prove the complete passenger outlet package clears the original glovebox body, hinges, latch and sweep, and prove the driver outlet package clears the selector contact stacks, terminals and loom. If either higher/inboard position conflicts, move that outlet outward within approved fixed fascia land while preserving the common horizontal datum; do not move the retained OEM parts or increase dashboard height.
