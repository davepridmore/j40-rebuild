...
| Rear wheel-cylinder teardown decision | Split after teardown | Workstream gate; no active generic order | May 24 photos plus Toyota/SOR references support a plausible four-cylinder rear setup, but this is not enough to order. Photograph/remove the old cylinders, then split exact purchase lines by fitted side/position/count if replacement is required, or mark the purchase redundant if existing assemblies are retained/rebuilt. Toyota `47550-69075`, `47560-69055`, `47570-69065`, `47580-69055`, Cruiser Corps `A28`, and SOR `081-02C-KIT` remain references only. |
| Rear shoes and spring/adjuster hardware | 1 four-shoe axle set plus the on-hand hardware pool | Correct-shoe purchase pending; non-friction hardware reported complete | The received MK `K-2255` is too wide and is rejected. Return/exchange for MK `K-2221` / Toyota `04494-60010` or `04494-60011`, nominally about `55–57 mm`, using seller ruler photos and the old sample. Count and lay out the on-hand pins, cups, springs, adjusters, levers and clips by LH/RH; buy only a proven missing/rejected piece. Verify drum ID, shoe arc/web ends/holes, parking-brake fit, movement and free return before release. |
| Rear drums | Side-specific after inspection | Workstream gate; no active pair order | Do not order now. Inspect and measure each side first; split into side-specific machine/replace work only if a drum fails inspection, otherwise mark the purchase redundant. Cruiser Corps `553-115` remains reference only. |
