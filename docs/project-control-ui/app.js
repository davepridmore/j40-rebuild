  ...
  function buildScoutCategories() {
    ...
    const fabricationExactSpecRows = [
      ...
      },
      {
        ...
        notes: "Current preferred route. Relay Rev D uses the existing covered relay box on a flat aluminium base with an exact insulating sheet, so no folded relay carrier or added cable-boot/loom-card fabrication is needed. Central battery positive enters the far-side cutoff stud first, then the near-side cutoff output splits to the MIDI fuse 4 input and the relay feed. MIDI Rev D is a hinged aluminium enclosure with one fuse 4 input grommet, five output grommets, and the far-side output hole enlarged for two power cables. The cutoff/kill switch sits beside the MIDI enclosure. Use the widened access-ladder placement shifted into the edge cavity by the configurable offset bars unless the cavity map proves a smaller, clear, and serviceable alternative.",
      },
      {
        ...
        spec: "Widened steel access ladder for the measured edge-cavity Relay Rev D flat-base pickup, existing MIDI Rev D enclosure and side kill-switch shelf pickups, protected cable support field, cutoff-switched branch feeds, and MIDI five-output fanout.",
        qty: "1",
        ...
        action: "Use as cardboard template first; only release final metal if the front/radiator-side cavity map proves the shifted edge-cavity tray/ladder position, about 80 mm relay-to-battery service gap, relay cover access, existing relay-box cable exit sweep, central battery-to-far-side-cutoff input routing, near-side cutoff-to-relay and cutoff-to-MIDI fuse 4 switched feeds, five-output MIDI cable bend clearance through the Rev D grommeted enclosure side, enlarged far-side two-cable output clearance, battery lift-out, and bonnet/radiator/hose clearance.",
      },
      {
        id: "BPCC-TRAY-001",
