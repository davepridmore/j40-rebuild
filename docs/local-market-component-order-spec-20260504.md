...
| `CLIP-LINE-01` | Hard-line support clips | On-hand mixed stock plus fasteners | On-hand rubber-lined plated or stainless P-clips plus edge/pass-through protection. | Clip IDs for `4.75 mm`, `6 mm`, and `8 mm` line OD matched to original bracket holes and measured line OD. | Match bracket holes, chafe points, and original hard-line route; add support only where the old route is missing/damaged or unsafe. | Plain cable ties, bare metal clips as permanent support, or moving the route to suit clip stock. |
...
Superseded 2026-06-01 by [Longman Rubber Order Spec](longman-rubber-order-spec-20260508.md). The rows below now mirror the active Longman single `80 x 80 x 24 mm` body-pad geometry where chassis/body rubbers are mentioned; the older circular body-pad route and old small/large square-pad split are retained only in legacy drawing files as reference.
...
| --- | --- | ---: | --- | --- | --- | --- |
| `BM-ISO-LG` | Single active square body isolator pad | `30 total` | Square flat body pad from one setup; order covers `12` base pads plus `18` stacking/trim spares where a station proves two simple `80 x 80` pads are needed or local trimming is required. | `80 L x 80 W x 24 H`; `18.0` centre bore for Toyota `90560-12009` style sleeve; plan corners `R1.5`; top/bottom edge break or chamfer `1.0` max; faces parallel `<=0.5`. | Black solid EPDM or NR/SBR automotive mount rubber, Shore A `60 +/-5`. | Released for quote/first article; confirm every station footprint, sleeve/cup/shim dry-stack, any two-pad stack, and compression before final install. |
| `BM-ISO-SM` | Removed small-height square body isolator pad | `0` | Do not quote or fabricate a separate `22 mm` body pad. | Removed; use `BM-ISO-LG` `80 x 80 x 24 mm` everywhere. | Same as active pad if a legacy sample is discussed. | Legacy traceability only; not an active order line. |
| `BM-CUP-SM` | Body-mount cup/seat washers | inspect existing first | Formed seat/cup washer, not a thin flat washer; reuse sound originals or recreate only from measured samples. | Must support the active `80 x 80` body pad and `18.0` bore/sleeve stack; M10 clearance about `11`; steel `2.5-3.0` thick unless originals prove otherwise. | Deburred and corrosion protected steel. | Do not order by old small/large rubber-pad split; confirm cup condition and station count by dry-fit. |
| `BM-CUP-LG` | Removed separate large/small cup ordering split | `0` | Do not fabricate a separate cup solely because of the old large-pad line. | Use measured original cup geometry under `BM-CUP-SM` inspection/reuse route. | Deburred and corrosion protected steel if remade. | Legacy traceability only; not an active order line. |
...
| `BUMP-F-L` | Front left bump stop | `1` | Fabricate or source exact long molded rubber assembly. Toyota `48304-60010` controls the long-stop family and `70 mm` free height; the photographed worn long-family sample also reads approximately `70 mm`. | Reproduce the May 31 broad rounded/tapered rubber body on its metal backing/fixture, with exposed metal mounting ears, central fixture/channel interface, and flat strike area. Photo-derived nominal values: metal-fixture length `~110`, rubber-body span along mounting-hole axis `~65`, and metal mounting-hole pitch `P ~90`. These nearest-`5 mm` values are for identification/first fit; release hole/thread `D`, transverse width, fixture/channel, strike-pad offset `X/Y`, loaded gap `G`, and full-bump clearance `F` by direct measurement. | New automotive NR/SBR bump-stop rubber, Shore A `70 +/-5`, with sample-proven fixture/insert retention where present; cast PU Shore A `80 +/-5` only if the same shape, metal fixture interface, and rebound recovery are held. | Fabricator must supply sketch/CAD, metal mounting-hole layout, fixture/channel trace, material declaration, and one first article; it must align by hand, capture the fixture/channel without tearing or rocking, contact within `+/-5`, survive 50 percent compression, and engage before shock/tyre/spring/brake-hose limits; no simple cut block. |
| `BUMP-F-R` | Front right bump stop | `1` | Fabricate or source exact short molded rubber assembly. Toyota `48304-60020` controls the short right-front family and `60 mm` free height. | Use the same May 31 metal-backed mounting arrangement and tapered/rounded rubber shape at the short height. Provisional photo values remain metal-fixture length `~110`, rubber span `~65`, and metal mounting-hole pitch `P ~90`; verify the right-front bracket and do not use the photographed `~70 mm` height here unless a full-bump test releases trimming. | Same compound family as left, but right-side geometry and height must stay distinct. | Reject aftermarket or locally made right-front stops at `70 mm` unless deliberately trimmed/released by vehicle full-bump check. |
| `BUMP-R` | Rear bump stops | `2` | Fabricate or source exact long molded rear/back rubber pair. Toyota `48304-60010` controls the long rear-stop family and `70 mm` free height. | Use the same May 31 metal-backed design made longer: photo-derived nominal metal-fixture length `~110`, rubber span along mounting-hole axis `~65`, and metal mounting-hole pitch `P ~90`. Release each rear bracket fit, hole diameter/thread, fixture/channel, transverse width, contact offset, loaded gap, and full-bump clearance from direct measurement; rear pair height match `<=1`. | Matched rear NR/SBR bump-stop rubber pair, Shore A `70 +/-5`, with sample-proven fixture/insert retention where present; PU fallback only as a matched pair with proven same shape and rebound. | Check Ironman ride height and axle travel before mould release; reject simple blocks, universal stops, failed retention tests, or unmatched rear pair. |
