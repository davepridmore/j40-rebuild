...
| `BUMP-F-L` | Front left bump stop | `1` | Fabricate or source exact long molded rubber/flat-plate assembly. Toyota `48304-60010` controls the long-stop family and `70 mm` free height; public sources do not expose the Toyota mould drawing, so do not copy the decayed old rubber. | Reproduce Toyota-style tapered/radiused progressive rubber body bonded/captive to the supplied one-piece flat steel backing/saddle plate with flat rectangular strike face. Release flat-plate outline/thickness, bolt/stud pitch `P`, hole/thread `D`, bond face, strike-pad offset `X/Y`, loaded gap `G`, and full-bump clearance `F` from the supplied plate, vehicle bracket, and axle strike pad. | New automotive NR/SBR bump-stop rubber, Shore A `70 +/-5`, bonded/captive to coated flat steel plate; cast PU Shore A `80 +/-5` only if the same progressive geometry, flat-plate bond/captive construction, and rebound recovery are held. | Fabricator must supply sketch/CAD, flat-plate hole layout, material declaration, and one first article; it must seat flat, bolt on by hand, keep the rubber/steel bond or captive joint intact, contact within `+/-5`, survive 50 percent compression, and engage before shock/tyre/spring/brake-hose limits; no simple cut block. |
| `BUMP-F-R` | Front right bump stop | `1` | Fabricate or source exact short molded rubber/flat-plate assembly. Toyota `48304-60020` controls the short right-front family and `60 mm` free height. | Same supplied-flat-plate tapered-body/flat strike-face requirement as left, but released from the right-front supplied plate, vehicle bracket, and strike pad. Do not use the `70 mm` long-stop height here unless a full-bump test releases trimming. | Same compound family as left, but right-side geometry and height must stay distinct. | Reject aftermarket or locally made right-front stops at `70 mm` unless deliberately trimmed/released by vehicle full-bump check. |
| `BUMP-R` | Rear bump stops | `2` | Fabricate or source exact long molded rear rubber/flat-plate pair. Toyota `48304-60010` controls the long rear-stop family and `70 mm` free height. | Same supplied-flat-plate tapered-body/flat strike-face requirement; release each rear flat plate, bolt/stud pattern, bond face, contact offset, loaded gap, and full-bump clearance from the supplied pieces, rear vehicle brackets, and strike pads; rear pair height match `<=1`. | Matched rear NR/SBR bump-stop rubber pair bonded/captive to coated flat steel plates, Shore A `70 +/-5`; PU fallback only as a matched pair with proven flat-plate bond/captive construction. | Check Ironman ride height and axle travel before mould release; reject simple blocks, universal stops, failed retention/bond tests, or unmatched rear pair. |
