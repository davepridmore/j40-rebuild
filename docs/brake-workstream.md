...
- Current outstanding brake parts and Lahore/Pakistan options: [brake_outstanding_lahore_options_20260618.csv](../data/manual/brake_outstanding_lahore_options_20260618.csv).
- Current rear-brake focus plan: [rear_brake_focus_plan_20260618.csv](../data/manual/rear_brake_focus_plan_20260618.csv).
- Current Bilal Ganj / Montgomery Road sample fabrication kits: [bilal-ganj-sample-fabrication-kits-20260619.md](bilal-ganj-sample-fabrication-kits-20260619.md).
...
- Brake hard-line stock is now a received-candidate: the user reports the copper-coloured tube has arrived. Use it only after the label/spec proves automotive brake-rated `4.75 mm / 3/16 in` CuNi/Cunifer or coated brake tube. If it is bare/plumbing copper, refrigeration tube, or unlabelled tube, reject it for final brake hydraulics. May 14 installed-fitting photos plus the user flare side-view point to Toyota-style `double / inverted` flares, not ISO bubble flares, but final fabrication still requires a removed old line or opened seat to confirm the straight-on sealing face, fitting thread, and seat at each connection.
- Two flaring kits are now user-reported on hand. They are accepted only after physical check proves at least one true `4.75 mm / 3/16 in` double/inverted flare die and it makes clean practice flares on the selected tube. If a kit is only a generic `4-16 mm` set without `4.75 mm / 3/16 in`, or makes only single/ISO bubble flares, it does not release brake-line fabrication.
...
- Rear drum correction from 2026-07-22: the received PakWheels `K-2255-N/Y` has failed the physical rear fit because it is too wide. Preserve it for return/exchange and do not modify it. The controlled replacement target is MK `K-2221` / Toyota `04494-60010` or `04494-60011`, one four-shoe axle set, nominally about `55–57 mm` wide, subject to seller ruler photos and old-sample/backing-plate confirmation. Owner now reports the remaining non-friction drum hardware pool complete; counts, condition, LH/RH layout and functional return still need workshop validation.
...
- Brake booster correction from 2026-07-29: the booster has now been confirmed functionally fine and needs cosmetic tidying only. Close booster replacement/rebuild scouting. Clean and refinish it without opening or altering the shell joint, pushrod/free-play, clevis, boot, check valve/grommet, studs, or master-cylinder interface. Normal fitted brake checks remain part of final system assembly.
- Front disc replacement decision from 2026-05-29: replace/renew all visible Sumitomo front disc service parts. Pads, retaining hardware, front flex hoses, rotors, and calipers are all replacement/renewal scope; measurements now identify the correct parts rather than deciding whether old front parts can stay.

## 2026-06-18 Gmail And Parts Refresh

- Gmail OAuth/import was restored through the repo workflow and wrote `data/processed/generated/comms_ingest_2026-06-18_categorized.csv`, `data/processed/generated/gmail_project_messages_2026-06-18.json`, and `data/processed/generated/comms_ingest_2026-06-18_status.json`: `53` messages read, `27` categorized.
- Gmail found ToolsMart order `TM25776` fulfillment/review evidence for `LICOTA MADE IN TAIWAN FLARE NUT WRENCH 12 X 14`. Treat the `12 x 14` line wrench as a received-candidate shelf-check item, not an active purchase requirement. Still confirm actual size coverage for `10/11/12/14/17 mm` before loosening old hydraulic fittings.
- Gmail also found an AliExpress `3/16 in` flaring-tool browsing/reminder email. That is marketing only and is not order evidence.
- User update 2026-06-18 says two flaring kits are on hand and the copper-coloured tube has arrived. Follow-up image clarification identifies the top part where the brake pipe enters the drum as the rear wheel cylinder, not a loose flare fitting. This reduces wheel-cylinder shopping pressure only if the received parts match that cylinder; it does not release hard-line tube nuts/fittings.
- User update 2026-06-19 says the received rear wheel cylinders seem to match exactly. Treat them as likely matched stock, still pending final count/side/position, bore, port thread/seat, mounting, pushrod, boot, and bleed-screw checks during teardown.
- Order check 2026-06-19: no standalone paid/ordered flare-nut connector line was found. The Altivox `25 ft` brake-line kit was ordered with `28 fittings`; those fittings may cover the need only after the physical package is counted and sample-matched to the old Toyota flare threads/seats.
- User update 2026-06-19 confirms the longer handbrake cable is on hand, but wheel-feed/equalizer/clevis/clip/adjuster pieces are still likely needed to connect and apply it at both rear wheels. Replace all handbrake return/assist springs rather than reusing weak old springs unless a new matching spring is proven included in the cable or Centric kit.
- User update 2026-06-19 originally marked the rear shoes on hand; the 2026-07-22 fit check supersedes that acceptance. K-2255 is on hand but rejected. Buy a brake-rated `3/16 in / 4.75 mm` flare tube-nut set as already directed, keep the parking-brake cable on the definite handbrake check list, and use [rear-brake-shoe-fitment-and-purchase-control-20260722.md](rear-brake-shoe-fitment-and-purchase-control-20260722.md) for the K-2221 exchange/purchase. Drum ID and service-limit measurement remain required.
- Handbrake buying links and source routes are now captured in [rear_handbrake_buy_links_20260619.csv](../data/manual/rear_handbrake_buy_links_20260619.csv). Use those links as sourcing routes; installation still depends on old-sample match and safe return/release.
- User clarification 2026-06-19: the useful target is the rear parking-brake splitter/equalizer cable assembly at the back, where one pull splits to both rear wheels. Treat it as a local auto-control-cable fabrication job, not an imported catalog hunt. Take the old assembly, yokes, clevises, pins, clips, springs, and photos to a Montgomery Road cable fabricator; use Bilal Ganj only for missing metal equalizer/yoke hardware by sample.
- User direction 2026-06-19: construct separate physical sample kits for the Bilal Ganj / Montgomery Road run: rear handbrake cable copy, rear brake/handbrake spring copy, brake flex hose samples, and fuel hose copies. Brake flex hoses require brake-rated crimped hydraulic assemblies; fuel rating applies to the fuel hoses only.
- Outstanding brake-system buys/checks are now summarized in [brake_outstanding_lahore_options_20260618.csv](../data/manual/brake_outstanding_lahore_options_20260618.csv). The immediate rear-brake blockers are rear flex hose, rear hard-line fabrication checks, brake-rated flare tube-nut purchase/sample match, drum hardware validation, parking-brake cable/complete handbrake wheel-end hardware/springs, and axle-side tidy/clip routing while the brakes are apart. Front pads/hardware/caliper/rotor matching is parked for now. The booster/servo is functionally confirmed good and no longer a sourcing or rebuild item; only cosmetic tidying remains. P-clips, U-clips, tees, the tube, wheel cylinders, shoes, and flaring kits are received/on-hand candidates until fit and safety checks pass.

## 2026-06-18 Rear Brake Focus

- Current active focus is rear brakes: rear axle hard lines, rear center flex hose, rear wheel cylinders, rear shoes, rear drum hardware, rear drums, parking-brake cable fit, parking-brake attachment hardware/springs, rear line/cable supports, and axle-side cleanup while the brakes are apart.
- Front disc pads, front calipers, front rotors, and front flex hoses remain documented replacement scope, but are parked from the immediate shopping/work focus unless inspection exposes a direct safety dependency.
- Brake booster/servo repair and replacement scouting is closed. The owner confirmed on 2026-07-29 that the existing booster is fine; only non-invasive cosmetic tidying remains before refit. Do not alter its pushrod/free-play or sealed functional interfaces.
- The part visible at the top of the rear drum/backing plate where the hard pipe enters from behind is the rear wheel cylinder / hydraulic entry-point assembly. It is not a tube nut, union, or brake-line connector. The received copies reportedly match exactly, so buy no duplicates unless teardown finds a count, side/position, port, bore, or mounting mismatch.
- Rear-brake execution should follow [rear_brake_focus_plan_20260618.csv](../data/manual/rear_brake_focus_plan_20260618.csv) before reopening broad brake-system purchases.
...
The May 29 picker import is classified as `engine_bay/brake_booster_servo_removed_sample` under the `brake_system` stage. These photos now serve as the pre-tidy condition and interface record. The 2026-07-29 owner confirmation supersedes the earlier replacement/rebuild assumption: the booster is functionally fine and needs cosmetic tidying only.
...
Current rear-brake buying status from the May 23 drum-open teardown chat plus the May 24 image/catalog check is therefore: replace worn/old hydraulics as baseline, but do not keep a generic rear wheel-cylinder order row. Rear cylinders stay in the brake workstream until teardown either splits them into exact fitted purchase lines by side/position/count, or proves the purchase is redundant because the existing assemblies are explicitly retained or rebuilt. The imported catalog routes below are spec references, not approval to import. Before buying these rear drum parts, get the cheap brake-line quote in [brake-line-cheap-sourcing-20260524.md](brake-line-cheap-sourcing-20260524.md).

| Item | Qty basis | Current status |
| --- | ---: | --- |
| Rear wheel cylinders / hydraulic entry-point assemblies | Actual count from teardown | Received likely-match stock per 2026-06-19 user update. After teardown, count and sample-match exact fitted cylinder positions by side/position/count; buy only missing, damaged, or mismatched positions. Toyota part numbers `47550-69075`, `47560-69055`, `47570-69065`, `47580-69055`, Cruiser Corps `A28`, and SOR `081-02C-KIT` remain references only. |
| Rear brake shoes / friction pieces | 1 four-shoe axle set | `K-2255-N/Y` from order `1075298` is rejected as too wide. Return/exchange it and obtain MK `K-2221` / Toyota `04494-60010` or `04494-60011`; GariParts lists `K-2221-N/Y` locally. Require seller ruler/box photos and old-sample match before payment, then verify arc, web ends, holes and drum fit before installation. |
| Rear drum hardware | 1 axle kit | Owner reports the Centric/collected/reconditioned hold-down, adjuster, lever, spring and clip pool complete as of 2026-07-22. Close further buying unless the left/right layout proves a missing or rejected piece; validate counts, spring/pin condition, adjuster/lever movement, routing and complete free return during one-side-at-a-time assembly. |
...
| Rear brake drums | Side-specific after inspection | No active pair order. Inspect and measure each side first; split into side-specific machine/replace work only if a drum fails inspection, otherwise mark the purchase redundant. Cruiser Corps `553-115` remains reference only. |

Still source locally/by sample: brake-rated `3/16 in / 4.75 mm` flare tube nuts as spare stock, the rear parking-brake splitter/equalizer cable assembly by local cable fabrication, rear parking-brake wheel-end/equalizer hardware, all handbrake return/assist springs not proven new, clips/retainers, and any cable/equalizer hardware not proven in the old samples. The rear axle hard-line tube stock is user-reported as arrived, but it is not fabrication-released until material/rating, flare/thread/seat, and old-route template checks are complete. Hydraulic-opening consumables are managed from on-hand/workshop supplies, not a separate order line; verify caps/plugs, cleaner, rags, and catch tray are ready before opening lines. Axle-side cleanup is a do-while-apart task only: clean, inspect, protect brackets/backing-plate exterior, and avoid contaminating drums, shoes, flare seats, bleeders, threads, or rubber boots.
...
| P0 | Rear axle brake T-union/splitter and any disturbed inline unions/splitters if corroded, seized, damaged, or mismatched. Brake tees/unions are on hand; use them only if they match the old location, thread, flare seat, port orientation, mounting style, and tube entry angles. |
...
| P0 | Brake flex-hose retaining U-clips and bracket hardware for front left, front right, and rear center hose brackets. U-clips are on hand; fit by original bracket/tab location and selected hose groove before buying any replacement. |
| P0 | Full line-support pack: rubber-lined P-clips for `4.75 mm` hard line, clamp-on rubber-lined axle supports, pass-through grommets/edge trim, M5/M6 fasteners, and larger cable saddles where parking-brake cables rub. P-clips are on hand; copy original support locations first and add support only where the old route is missing/damaged or unsafe. |
...
| P1 | Brake-line clips, parking-brake cable retainers, rubber sleeves/grommets, and rubber-lined P-clips where originals are missing or corroded; use on-hand stock at the same route locations before any new purchase. |
...
| Item | Why held |
| --- | --- |
| Rear brake shoes / friction pieces | New shoes are user-reported on hand, but fit is not closed until units are counted and matched for shoe width, arc, lining, web holes, adjuster/parking-brake lever contact points, and drum fit. |
...
| Rear wheel cylinders / hydraulic entry-point assemblies | Received parts reportedly match exactly, so do not buy duplicates. Final release still waits for teardown count, side/position, bore, mounting, port thread/seat, pushrod, boot, and bleed-screw access checks. |
| Front pads/calipers/rotors | Replacement/renewal is approved and the front calipers are now visibly Sumitomo fixed type, but exact pads, rebuild-vs-rebuilt-caliper path, and replacement rotors still wait for pad outline, cleaned casting marks, piston dimensions, and rotor measurements. |
| Rear parking-brake cables | Cable is a definite handbrake item. Use the on-hand longer cable only if it proves to be the complete required cable section(s); otherwise buy missing cable sections by old sample. Cable length, sheath length, end fittings, equalizer hardware, spring layout, and clip positions must come from the old samples. |
...
| --- | --- | --- |
| Rear parking-brake cables | Label left/right; photograph both ends before removal; measure overall cable length, sheath length, backing-plate end type, equalizer/intermediate end type, bracket/clip positions, adjuster thread size, and adjuster travel. Keep both old cables intact if possible. | Rear parking-brake cable set, equalizer hardware, clevises, clips, return springs, and wheel-feed parts. |
| Equalizer / intermediate linkage | Photograph the handle-to-equalizer/intermediate layout, clevis pins, return springs, adjuster, cable attachment points, and rear wheel lever/cable stops. Record missing or seized parts. | Cable kit completeness, wheel-end feed hardware, replacement handbrake springs, and handbrake adjustment hardware. |
...
| Rear drums open | Photograph each side before removing springs; record drum inside diameter, shoe width, lining condition, adjuster style, spring layout, backing-plate condition, parking-brake lever condition, wheel-end cable hardware, and drum scoring/step. | Rear shoes, hardware kit, adjusters, handbrake springs, wheel-end cable hardware, drum machine/replace decision, and axle-side tidy scope. |
...
| Clips and retainers | Count every brake-line clip, parking-brake cable retainer, rubber sleeve/grommet, and P-clip by position; keep one sample of each style; record hole size, bracket thickness, and line/cable OD. | Clip order, P-clip pack, permanent line/cable support, and axle-side route tidy. |
...
- Exact rear shoe release and final wheel-cylinder installation wait until internal drum photos and measurements are captured; wheel cylinders are likely on hand, not a duplicate buy item unless teardown proves mismatch.
- New rear shoes are on hand, so do not duplicate-buy shoes unless the shoe width/arc/layout check fails.
- Buy spare brake-rated flare tube nuts now, but install only after old-sample thread/seat match.
- Exact front pad, front hose, caliper renewal, and rotor replacement orders use the visible Sumitomo fixed-caliper baseline but still wait for pad outline, cleaned casting marks, piston/rotor measurements, fitting threads/seats, free length, and steering-lock clearance.
...
- Axle-side tidy work must happen before final assembly and must not hide leaks or contaminate friction surfaces, flare seats, bleeders, threads, or rubber boots.
