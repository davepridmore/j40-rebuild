...
- Brake hard-line stock is now procured by Amir as `25 ft` of zinc-coated/galvanised `3/16 in` brake tube. Use it only after receipt/spec check proves automotive brake-rated `4.75 mm / 3/16 in` coated tube. May 14 installed-fitting photos plus the user flare side-view point to Toyota-style `double / inverted` flares, not ISO bubble flares, but final fabrication still requires a removed old line or opened seat to confirm the straight-on sealing face, fitting thread, and seat at each connection.
...
| --- | ---: | --- |
| Rear wheel cylinders / hydraulic entry-point assemblies | Actual count from teardown | Workstream split/redundancy gate, no active purchase row. After teardown, split into exact fitted cylinder order lines by side/position/count if replacement is required; otherwise mark the purchase redundant if the old assemblies are proven serviceable/rebuildable and explicitly retained. Toyota part numbers `47550-69075`, `47560-69055`, `47570-69065`, `47580-69055`, Cruiser Corps `A28`, and SOR `081-02C-KIT` remain references only. |
...
Still source locally/by sample: brake flare nuts/fittings, parking-brake cable attachment hardware, clips/retainers, and any cable/equalizer hardware not proven in the old samples. The rear axle hard-line tube stock is procured by Amir but not fabrication-released until receipt/spec, flare/thread/seat, and old-route template checks are complete. Hydraulic-opening consumables are managed from on-hand/workshop supplies, not a separate order line; verify caps/plugs, cleaner, rags, and catch tray are ready before opening lines.
