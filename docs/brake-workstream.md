...
- User update 2026-06-19 adds three decisions: buy a brake-rated `3/16 in / 4.75 mm` flare tube-nut set anyway, keep the parking-brake cable itself on the definite handbrake buy/check list, and mark the new rear brake shoes as on hand. The simple drum cleanup is useful progress, but drum ID/service-limit measurement is still required.
...
- Brake booster/servo work is a parallel service route, not a parts-shopping blocker for the rear axle. Preferred path is to service/rebuild the removed old booster using the video gate in [amir-refurbishment-video-gates-20260529.md](amir-refurbishment-video-gates-20260529.md). Use an exchange/refurbished unit only if the old booster cannot be rebuilt and the replacement sample-matches the old unit.
...
| Rear drum hardware | 1 axle kit | Centric `116971-05110530` is procured by Amir. Validate kit contents against the opened drum layout before installation: return springs, hold-down pins/cups/springs, adjuster hardware, retaining clips, parking-brake lever clips, shoe width/web holes, and side-to-side symmetry. All rear drum/handbrake springs should be replaced; any external cable/lever spring not included in this kit stays under `BR-REAR-009`. Cruiser Corps `551-113`, `A16`, and SOR references are only to identify contents and compare layout. |
