...
All replacement-pipe requirement rows are now defined, including the connector-hose row added after the rubber visual audit on 2026-05-03. The May 29 larger-pipe detail photos are now direct `COOL-D` evidence for the formed metal coolant pipe, connector hoses, clamps, and clips.
...
| Formed coolant pipe | `RPO-COOL-005` | Order one `28-30 mm OD`, `1.2-1.6 mm` wall tube blank; buy/quote `1000 mm` shop stock if sold by meter, with `750 mm` as the absolute minimum blank before bending/trimming. Beads `1.5-2.0 mm`; clamp lands `25-30 mm`. Old metal pipe is a replaceable pattern part. |
| Formed pipe connector hoses | `RPO-COOL-006A`, `RPO-COOL-006B` | Buy two `500 mm` EPDM connector blanks on `28-30 mm` ID exact order basis; cut by sample and dry-fit. Old connector hoses are pattern parts only. |
| Diesel rubber hoses | `RPO-FUEL-001A`, `RPO-FUEL-001B`, `RPO-FUEL-001C` | Buy `8 mm ID x 1500 mm` feed for the measured approx `1200 mm` tank-to-engine route, `6 mm ID x 2000 mm` return, and `3.2-3.5 mm ID x 1000 mm` leak-off diesel-rated stock. |
| Fuel hard lines | `RPO-FUEL-002A`, `RPO-FUEL-002B` | `8 mm OD x 5000 mm` feed pipe allowance is conditional only if a separate rigid feed line exists beyond the measured flexible feed hose; `6 mm OD x 5000 mm` return pipe allowance remains orderable. Final section lengths only after corrosion inspection and route measurement. |
...
| Supports | `RPO-CLIP-001` | Rubber-lined P-clips and pass-through protection ready; size to `4.75 mm`, `6 mm`, and `8 mm` measured line ODs with `300-400 mm` support spacing. Replace old/corroded clips; use old clips only as size/location patterns. |
...
3. Flat-lay the formed metal coolant pipe and record tube OD, wall, final centerline length, bend radii, clocking, beads, and clamp lands in `mm`; the orderable stock is `28-30 mm OD`, `1.2-1.6 mm` wall, `1000 mm` shop stock preferred or `750 mm` absolute minimum blank. Use the May 29 larger-pipe detail photos as supplemental tape/OD/end/clamp evidence.
...
6. Inspect hard fuel lines under clips and decide keep, section repair, or full replacement; order the `5000 mm` feed hard-line allowance only if a separate rigid feed line exists beyond the measured flexible feed hose, while the `5000 mm` return allowance remains orderable. Record final section lengths and clip spacing in `mm`.
...
- Do not treat the old formed metal pipe, connector hoses, clamps, or clips as final-use parts. They are patterns for new replacements.
