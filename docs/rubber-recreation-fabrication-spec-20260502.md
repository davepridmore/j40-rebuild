...
| `BUMP-F-L`, `BUMP-F-R`, `BUMP-R` bump stops | `bump_stop_vehicle_measurement_control.svg` | May 31 front-stop photo mould control. Photo-derived nominal dimensions are approximately `70 mm` long-sample height, `110 mm` metal-fixture length, `65 mm` rubber-body span along the mounting-hole axis, and `90 mm` metal mounting-hole pitch. Long `48304-60010` positions remain externally controlled at `70 mm`; right-front `48304-60020` remains `60 mm` unless vehicle testing says otherwise. Reproduce the molded rubber body on its metal backing/fixture with mounting holes in the exposed metal ears, central fixture/channel interface, broad rounded/tapered sides, and flat strike area. Rear/back stops use the same front-stop body and fixture pattern made longer. Photo values are nearest-`5 mm` identification/first-fit dimensions; hole diameter/thread, transverse width, fixture detail, and contact offset remain caliper/thread-gauge/vehicle controls. See `docs/bump-stop-fabrication-spec-20260504.md`. |
...
Bump stops are the exception to the body/front-support hardness target. Use the separate bump-stop material control: `NR/SBR` automotive bump-stop rubber Shore A `70 +/-5`, or cast automotive PU Shore A `80 +/-5` only if the May 31 metal-backed fixture-ear mounting layout, central fixture/channel interface, progressive mould shape, and rebound recovery are held. Rear/back stops use the same shape made longer. The photo-survey nominals (`~70 mm` long-family sample height, `~110 mm` fixture length, `~65 mm` rubber span and `~90 mm` metal-hole pitch) are identification/first-fit values to roughly the nearest `5 mm`; caliper the samples, fixture and threaded holder before mould release.
