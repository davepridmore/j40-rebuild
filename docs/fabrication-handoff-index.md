...
Current dimensional-control package: [V46-D1 A1 1:1 component PDF](../data/manual/fabrication/dashboard_lcd_hvac_fascia_rev_i/dashboard_rev_i_v46_d1_component_package_1to1.pdf), [editable 1:1 SVG](../data/manual/fabrication/dashboard_lcd_hvac_fascia_rev_i/dashboard_rev_i_v46_d1_component_package_1to1.svg), [dimension schedule](../data/manual/fabrication/dashboard_lcd_hvac_fascia_rev_i/dashboard_rev_i_v46_d1_component_dimension_schedule.csv) and [measurement-control specification](../data/manual/fabrication/dashboard_lcd_hvac_fascia_rev_i/dashboard_rev_i_v46_d1_measurement_control_spec.md). This sheet draws only manufacturer-published component envelopes at physical size. It is **NOT FOR CUTTING** and does not infer the centre-panel perimeter from photographs.

Current appearance direction: [V44 factory-height rounded-corner proposal](../data/manual/fabrication/dashboard_lcd_hvac_fascia_rev_i/dashboard_rev_i_v44_factory_height_rounded_top_corners_proposal.md), with the accepted [body-colour front-on design](../data/manual/fabrication/dashboard_lcd_hvac_fascia_rev_i/dashboard_rev_i_v44_factory_height_rounded_top_corners.png) and [boundary/mounting diagram](../data/manual/fabrication/dashboard_lcd_hvac_fascia_rev_i/dashboard_rev_i_v44_factory_height_rounded_top_corners_boundary.svg). They are non-dimensional and must not be scaled or traced. V43's sharp-corner study and V41/V42 are superseded boundary studies; V40 remains a historical oblique design-intent reference. Only a new M1/M2 vehicle trace can define the V44 production geometry.

V44 keeps the OEM glovebox and speedometer unchanged and forms one visibly closed, balanced centre panel between them. Its two upper corners use smooth OEM-style radii matching the neighbouring openings, its side seams close at those OEM boundaries, and its bottom remains the factory lower line with 0 mm drop. The panel consumes the ashtray, central rectangular opening and lower slot without continuing to the far-right dashboard end. It holds the purchased Pioneer DMH-AP6650BT flush, plus one line of seven bought black long-lever selectors and a separate red HAZARD. All perimeter coordinates, radii, component apertures and rear packaging remain HOLD pending physical traces, actual components and a rigid full-depth rear buck.

Portal/CNC handoff: use the [measurement and survey checklist](../data/manual/fabrication/dashboard_lcd_hvac_fascia_rev_i/cnc_measurement_and_survey_checklist.md) and [fillable M1–M10 schedule](../data/manual/fabrication/dashboard_lcd_hvac_fascia_rev_i/cnc_measurement_schedule.csv). M1–M9 release the measured fabrication geometry and physical fit; M10 closes post-fit electrical/functional commissioning before vehicle handoff. Use V46-D1 to compare component packages and record the missing data, not as a panel outline. The V44 rendering and every earlier image/elevation/specification are prototype inputs only and must not be used to cut the vehicle or production fascia.
