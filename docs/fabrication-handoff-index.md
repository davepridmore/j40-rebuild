...
Current geometric appearance baseline: [dashboard_rev_i_v32_inboard_high_87mm_outlets.png](../data/manual/fabrication/dashboard_lcd_hvac_fascia_rev_i/dashboard_rev_i_v32_inboard_high_87mm_outlets.png), with the [owner's outlet photograph](../data/manual/fabrication/dashboard_lcd_hvac_fascia_rev_i/supplied_ac_outlet_87_75_22_reference.png) controlling the provisional vent appearance and the [owner's bought-selector photograph](../data/manual/fabrication/dashboard_lcd_hvac_fascia_rev_i/bought_long_handle_selector_reference.png) controlling the switch appearance.

Rev I targets the original shallow full-width dashboard height, transfers the original large asymmetric plate-and-knob Toyota glovebox and complete speedometer cluster at their directly traced original positions, deletes and closes over the separate smaller ashtray, and positions the cluster immediately above the original steering-column shroud on their common vehicle-traced axis. V32 centres the provisional 198.91 × 111.89 mm active image of a nominal 9-inch LCD on the complete fascia and shows exactly two Ø87 mm target outlet faces slightly higher and inward on one common datum. Those positions are visually selected only: their physical centres, the Ø75 mm target mounting interface and the approximately 22 mm target louver-body projection remain subject to the signed trace, all-four-part measurement, same-finish coupon and full-depth mock-up. The steering opening's X/Y and outline remain immutable and must be direct-traced at M3; no render is a dimensional drawing. The actual outlet retainer, spigot, clamp, first bend, duct, glovebox depth and opening sweep require at least 10 mm verified clearance. If a higher/inboard rear package conflicts, move its outlet outward within signed fixed fascia land rather than moving an OEM assembly or increasing dash height. The seven selector heads follow the owner's photographed round chrome-bezel, black-face, long-black-lever controls, with the square bodies concealed behind the fascia. The received inventory is exact: 3-position WIPERS/LIGHTS/BLOWER and 2-position SPOTS/AUX/A/C/FUEL STOP, plus a separate red HAZARD; no four-position selector or additional selector purchase is required. Production vehicle cutting and final component apertures remain HOLD until M1–M10 are signed.

Portal/CNC handoff: use the [measurement and survey checklist](../data/manual/fabrication/dashboard_lcd_hvac_fascia_rev_i/cnc_measurement_and_survey_checklist.md) and [fillable M1–M10 schedule](../data/manual/fabrication/dashboard_lcd_hvac_fascia_rev_i/cnc_measurement_schedule.csv) for an exact production release. The separate [V32-Q1 assumption-based specification](../data/manual/fabrication/dashboard_lcd_hvac_fascia_rev_i/dashboard_rev_i_v32_provisional_assumption_cnc_spec.md) and [provisional coordinates](../data/manual/fabrication/dashboard_lcd_hvac_fascia_rev_i/dashboard_rev_i_v32_provisional_coordinates.csv) are quotation/prototype inputs only and must not be used to cut the vehicle or production fascia.
