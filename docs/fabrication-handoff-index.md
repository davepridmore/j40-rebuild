...
Current appearance baseline: [layout_b_column_v15_momo_two_outer_vent_oem_glovebox_ashtray_delete_original_height_centered_lcd_straight_on.png](../data/manual/fabrication/dashboard_lcd_hvac_fascia_rev_h/layout_variants_20260801/layout_b_column_v15_momo_two_outer_vent_oem_glovebox_ashtray_delete_original_height_centered_lcd_straight_on.png)

Rev I targets the original shallow full-width dashboard height, transfers the original large asymmetric plate-and-knob Toyota glovebox and complete speedometer cluster, deletes and closes over the separate smaller ashtray, positions the cluster immediately above the original steering-column shroud on their common vehicle-traced axis, centres a true 9-inch active image on the complete fascia face, uses exactly two high outer/end circular outlets, and retains seven black selectors plus the separate red hazard. V15 is a visual target, not a dimensional drawing. It is ready for physical tracing, bought-component measurement, full-size/full-depth mock-up and CNC quotation only. Production vehicle cutting and final component apertures remain HOLD until M1–M10 are signed.
