...
- Relay field carries Relay Rev D outside the battery footprint on the outboard/access edge of the front/radiator-side service face: flat `360 x 245 x 3 mm` aluminium base mounted vertically and directly flat to the main metal service sheet, exact `300 x 197 x 3 mm` insulating sheet against the relay box's large uncovered bottom face, and the existing covered relay box facing a removable-cover service path. Do not add separate relay cable-boot, output-cluster, or loom-card fabrication above the covered box.
...
- Central battery positive -> far-side side-mounted cutoff/kill-switch stud first. The near-side cutoff output then splits into two protected branches: one to the MIDI fuse 4 input and one red relay power pipe into the relay top hole closest to the battery. The relay output power bundle leaves the second relay top hole.
...
- Stand/tray cardboard mock-up with standard `318 x 180 x 230 mm` battery envelope, central top terminals, removable hold-down, battery lift-out path, adjustable offset bars at `160 / 190 / 230 mm`, Relay Rev D vertical flat base and exact insulating sheet outside the battery footprint on the outboard/front access edge with about `80 mm` relay-to-battery service gap, red relay power pipe into the top hole closest to the battery, relay output power bundle from the second top hole, MIDI Rev D hinged enclosure on its separated top-front shelf with fuse 4 input grommet, five output grommets, and the far-side output hole enlarged for two power cables, folded cutoff/kill-switch base/guard beside the MIDI enclosure with far-side battery input, central battery-to-far-side-cutoff input path, near-side cutoff-to-relay and cutoff-to-MIDI branches, five MIDI output cables, and cable lugs clears the bonnet, radiator, fan/belt, LHD steering shaft/box/service path, alternator service sweep, and cable bend radius.
...
- Use the installed battery as the fixed exclusion block before placing electrical hardware. May 17 photos now provide the actual Daewoo DLS120 battery ruler references; transcribe the final case length, width, height, terminal height, and hold-down path into the cavity map before relying on the previous standard `318 x 180 x 230 mm` comparison envelope.
...
8. Proposed relay/fuse/cutoff mock-up: widened access-ladder cards held with battery installed: `660 x 310 mm` service spine, `340 x 265 mm` tray and `318 x 180 x 230 mm` battery envelope with central terminals, adjustable offset bars at `160 / 190 / 230 mm`, Relay Rev D vertical flat base `360 x 245 mm` with exact `300 x 197 mm` insulating sheet and the existing covered relay box outside the battery footprint, red relay power pipe into the relay top hole closest to the battery, relay output power bundle from the second top hole, MIDI Rev D `210 x 165 x 65 mm` top-front hinged enclosure with fuse 4 input grommet, five output grommets, the far-side output enlarged for two power cables, and five output cables, folded cutoff/kill-switch base/guard `170 x 110 mm` finished face / `210 x 150 mm` flat pattern / `20 mm` upward lips beside the MIDI enclosure with far-side battery input, central battery-to-far-side-cutoff and near-side cutoff-to-relay/MIDI cable lug sweeps, and the LHD steering-side no-go envelope.
9. Cutoff switch or isolator body: panel-hole size, body depth, terminal stud size, and cable exit directions.
10. Cable exit route from central battery positive to far-side cutoff stud, then near-side cutoff output to MIDI fuse 4 and to the red relay power pipe entering the relay top hole closest to the battery, relay output bundle from the second top hole, plus MIDI outputs toward the harness.
