...
- Battery positive -> master cutoff -> two direct protected branches: one to the MIDI common feed and one to the covered relay common feed.
...
- Trial cardboard templates as a stack: widened front access-ladder spine `660 x 310 mm`, `340 x 265 mm` battery tray with `318 x 180 x 230 mm` battery envelope, Relay Rev C outboard/front `320 x 220 mm`, relay plastic rear guard/underlay `280 x 185 mm`, sized upper/side/lower relay exit cards, MIDI Rev C top-front `190 x 150 mm` with seated output comb/backplate and enlarged double-wire output access hole started from the battery leading-edge datum, folded cutoff base/guard `170 x 110 mm` finished face / `210 x 150 mm` flat pattern / `20 mm` upward lips started from the same datum, plus depth blocks for direct cutoff branches, relay cover removal, cable lugs, MIDI five-output fanout, and boots.
