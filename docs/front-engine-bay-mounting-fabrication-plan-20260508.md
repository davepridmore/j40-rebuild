...
This plan splits the vague "missing bracket" concern into concrete front engine-bay fabrication jobs: radiator two-side retention, battery tray support, and a chassis-mounted integrated power carrier for the known relay base, MIDI Rev D hinged enclosure/subplate, and battery master cutoff.
...
Preferred outcome: a removable steel chassis-mounted stand that supports a full-height battery and carries the known relay carrier base, MIDI Rev D fuse-holder enclosure/subplate, and battery master cutoff/isolator base as one serviceable assembly.
...
- Relay field carries the existing relay carrier base outside the battery footprint on the outboard/access edge of the front/radiator-side service face, rotated to a `220 x 320 mm` finished face from the same folded-tray basis, with the `185 x 280 mm` flat plastic rear guard/underlay placed before the folded metal tray, the covered relay box facing a removable-cover service path, top cutoff input, top large power-output cluster, and end-side input/cabin loom relief left clear.
...
- Central battery positive -> far-side side-mounted cutoff/kill-switch stud first. The near-side cutoff output then splits into two protected branches: one to the MIDI fuse 4 input and one to the rotated relay top power input.
- Small-wire bundle routes underneath the MIDI switches to the top of the relay. Relay end-side inputs split off with the majority expected to continue toward the cabin.
...
- Stand/tray cardboard mock-up with standard `318 x 180 x 230 mm` battery envelope, central top terminals, removable hold-down, battery lift-out path, known relay base rotated outside the battery footprint on the outboard/front access edge with about `80 mm` relay-to-battery service gap, plastic guard/underlay before the folded metal tray, relay top cutoff input and large power-output cluster, relay end-side input/cabin loom exit, MIDI Rev D hinged enclosure on its separated top-front shelf with fuse 4 input grommet, five output grommets, and the far-side output hole enlarged for two power cables, folded cutoff/kill-switch base/guard beside the MIDI enclosure with far-side battery input, central battery-to-far-side-cutoff input path, near-side cutoff-to-relay and cutoff-to-MIDI branches, small-wire route under MIDI to relay top, five MIDI output cables, and cable lugs clears the bonnet, radiator, fan/belt, LHD steering shaft/box/service path, alternator service sweep, and cable bend radius.
...
- The bridge must be adjustable in height and side-jog during mock-up. If the `180 mm` rise or `190 mm` outboard jog moves the battery out of the original pocket, blocks bonnet/terminal clearance, or enters steering/hose service space, reset from the measured battery pocket rather than forcing the estimate.
...
8. Proposed relay/fuse/cutoff mock-up: widened access-ladder cards held with battery installed: `660 x 310 mm` service spine, `340 x 265 mm` tray and `318 x 180 x 230 mm` battery envelope with central terminals, rotated relay `220 x 320 mm` outboard/front face outside the battery footprint, `185 x 280 mm` relay plastic rear guard/underlay before the folded metal tray, relay top cutoff input and large power-output cluster, relay end-side input/cabin loom exit, MIDI Rev D `210 x 165 x 65 mm` top-front hinged enclosure with fuse 4 input grommet, five output grommets, the far-side output enlarged for two power cables, and five output cables, folded cutoff/kill-switch base/guard `170 x 110 mm` finished face / `210 x 150 mm` flat pattern / `20 mm` upward lips beside the MIDI enclosure with far-side battery input, central battery-to-far-side-cutoff and near-side cutoff-to-relay/MIDI cable lug sweeps, small-wire under-MIDI route to relay top, and the LHD steering-side no-go envelope.
9. Cutoff switch or isolator body: panel-hole size, body depth, terminal stud size, and cable exit directions.
10. Cable exit route from central battery positive to far-side cutoff stud, then near-side cutoff output to relay and MIDI fuse 4, small-wire route under MIDI to relay top, relay end-side inputs toward the cabin, and MIDI outputs toward the harness.
...
The removable power-carrier backplane, relay base, MIDI enclosure, non-conductive subplate, cutoff switch, and final cable clips can be installed after coating, provided the formed chassis saddle/upright structure is already installed, deburred, primed, and protected.
