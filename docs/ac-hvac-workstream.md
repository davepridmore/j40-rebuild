...
- Consolidate exactly seven bought long-handle selectors plus a separate red hazard in a compact two-row bank at the extreme right. Use four nominal columns at `X=1096/1144/1192/1240 mm`, top row `Y=88 mm` and bottom row `Y=30 mm` (48 mm horizontal and 58 mm vertical pitch). Top row is `WIPERS | LIGHTS | SPOTS | AUX`; bottom row is `BLOWER | A/C | FUEL STOP | HAZARD`. `WIPERS`, `LIGHTS`, `BLOWER` are 3-position; `SPOTS`, `AUX`, `A/C`, `FUEL STOP` are 2-position. `FUEL STOP` is a low-current RUN/STOP request subject to EEI-003, authoritative key-OFF shutdown and the retained manual stop cable. Keep cabin thermostat/temperature/blend with the delivered evaporator controller, outside this seven-selector count.
...
The owner now prefers a smaller cabin package and a factory-height fascia with exactly two occupant outlets. The active front-face target is the [Rev I V35 specification](../data/manual/fabrication/dashboard_lcd_hvac_fascia_rev_i/README.md): only the ashtray/centre area is replaced by a removable centre cassette. The OEM glovebox, speedometer/cluster, RHD column scallop, full-width lower edge and all existing formed contours remain unchanged. The cassette uses zero extension/drop by default; any later extension is centre-only and requires physical measurement plus owner approval. This supersedes the Rev H four-outlet distribution and the misidentified passenger-side forms in V13/V14.

- Retain two matched, generously sized directional outlets centred in retained measured outer clear lands, one aimable at the passenger and one at the driver. The V35 target is Ø87 visible face, Ø75 rear mounting interface and ≤ approximately 22 mm louver-body projection; those values control the visualization and fit-coupon brief only until all four ordered vents are measured. Delete the Rev H inner/lower pair and its pods. Use two supported end branches and prove useful balanced flow, bend radius and service clearance in the full-depth rear buck.
- The [2026-08-02 rear-clearance photo audit](../data/manual/fabrication/dashboard_lcd_hvac_fascia_rev_i/rear_clearance_photo_audit_20260802.md) records two oblique local rays of approximately 125–130 mm and 160–170 mm. A third photograph shows a 270–280 mm endpoint reading, not a span, because its tape zero/start is outside the frame. This is enough to say the approximately 22 mm vent body is not contradicted at the photographed local ray, but the views are not registered to either actual vent centre and do not show the retainer, spigot, clamp, hose or first bend. Complete vent fit therefore remains HOLD pending perpendicular depth maps at V1/V2 and an actual-parts rear buck.
- For LCD quotation only, use a maximum **115 mm complete installed projection** from the finished rear fascia, with a target **≤80 mm integral LCD chassis/body including any integral rear heat sink** and at least 35 mm retained inside the 115 mm envelope for carrier, mounts, fasteners, largest mated connector, cable bend/service loop and removal tolerance. The generic 5.0 mm panel thickness is not installed depth. For production replace the quotation number from M5 measurements and then validate the complete package at M9: `P_LCD_INSTALLED,PROD ≤ min[Z_FIXED(x,y)] − 10 mm`, while maintaining at least 20 mm to the moving column/shroud/stalk envelope.
- Place one line of seven bought black long-lever selectors plus a separate red HAZARD below the LCD on the cassette. The actual heads, keys, lever sweeps, rear contact stacks and wiring remain HOLD M6/rear buck.
- Keep the outlet interfaces separate: Ø75 rear mounting is neither a production panel aperture nor a hose-neck size. Measure the spigot OD/ID/projection/bead and complete retainer/clamp/first-bend package, then use a sealed smooth adapter between that spigot and the evaporator's proven duct standard.
...
- Use the bought 3-position `BLOWER` selector only as an `OFF / LOW / HIGH` input to the measured resistor/PWM/relay arrangement and the 2-position `A/C` selector only as a compressor request through thermostat and pressure safeties. Leave the donor medium-speed tap unused unless a separately documented and bench-proven controller redesign is approved. Retain or separately mount the delivered unit's thermostat/temperature/blend controller. The `FUEL STOP` selector is outside the HVAC circuit and remains electrically on HOLD until EEI-003 proves the correct fail-safe interface.
