...
The owner now prefers a smaller cabin package and a factory-height fascia with exactly two occupant outlets. The active front-face target is the [Rev I V32 specification](../data/manual/fabrication/dashboard_lcd_hvac_fascia_rev_i/README.md): the LCD active image is centred on the complete fascia face, while the two unchanged-size outlet faces sit slightly higher and inward on one visual datum. Their physical centres are positioned independently on measured fixed lands after the rear packages are mocked up, never by scaling the render. Retain the original large asymmetric plate-and-knob glovebox and speedometer at their directly traced positions, delete and close over the separate smaller ashtray, and preserve the factory column relief and control-bank decisions in Rev I. This supersedes the Rev H four-outlet distribution and the misidentified passenger-side forms in V13/V14.

- Retain two matched, generously sized directional outlets high in the fixed outer regions, one aimable at the passenger and one at the driver. The current V32 target is Ø87 visible face, Ø75 rear mounting interface and ≤ approximately 22 mm louver-body projection; those values control the visualization and fit-coupon brief only until all four ordered vents are measured. V32's slightly higher/inboard presentation is accepted visually, subject to the glovebox and selector-side rear-envelope checks; shift outward within signed fixed land if needed. Delete the Rev H inner/lower pair and its pods. Use two supported end branches and prove useful balanced flow, bend radius and service clearance in the full-depth mock-up.
