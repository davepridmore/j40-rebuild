...
- Plan for four identical circular satin-silver outlets integrated into the Rev H dashboard: one close to each usable flat-face end and an inner pair flanking the central LCD. Align all four `Ø87 mm` bezel tops with the LCD-bezel top (nominal proportional-model top datum `Y=212.0 mm`; vent centres `Y=168.5 mm`). The LCD centre, fascia centreline and midpoint of the two inner vents are one shared CNC datum; the inner vents are mirrored at equal offsets. Keep clear metal between each inner vent and the LCD, and do not put the end pair in the side returns or A-pillars.
...
- Keep all four vent faces wholly within the main fascia; the raised outlet position removes the need for lower vent lobes or a vent channel. Use only one compact lower driver-side control channel for the selector/contact-stack depth. Mock all four supported duct branches and prove clearance to the retained glovebox, original cluster, steering column, LCD connectors, selector contact stacks and service paths.
- Consolidate exactly seven Schneider selectors plus a separate red hazard in one compact horizontal row within the driver-side channel, at `39 mm` nominal pitch. Passenger-to-driver order is `WIPERS | LIGHTS | SPOTS | AUX | BLOWER | A/C | ENGINE | HAZARD`: `WIPERS`, `LIGHTS`, `BLOWER` are 3-position; `SPOTS`, `AUX`, `A/C`, `ENGINE` are 2-position. `ENGINE` uses the former spare position as a low-current RUN/STOP request subject to EEI-003, authoritative key-OFF shutdown and the retained manual stop cable. Keep cabin thermostat/temperature/blend with the delivered evaporator controller, outside this seven-selector count.
